...
//                    await
//                        modelContainerGrain.SimpleSelectMany(
//                            model => model.RootElements.Single().As<RailwayContainer>().Descendants().OfType<ISwitch>(), factory, 2);
...
//                    action: sw =>
//                    {
//                        Console.WriteLine(sw.RelativeUri);
//                        return modelContainerGrain.ExecuteSync((container, elementUri) =>
//                        {
//                            Console.WriteLine("Start " + (Uri) elementUri);
//                            var swi = (ISwitch) container.Resolve((Uri) elementUri);
//                            swi.Sensor = new Sensor();
//                            Console.WriteLine("End " + (Uri) elementUri);
//                            //Console.WriteLine("End: SensorID: {0}", swi.Sensor.RelativeUri);
//                        }, sw.RelativeUri, true);
//                    },
...
//                        var localSwitch = (ISwitchPosition) model.Resolve(localMatch.Item2);
//                        var localSensor = (ISensor) model.Resolve(localSwitch.Switch.Sensor.RelativeUri);
...
//                    (model, route) => new ModelElementTuple<Model, IRoute>(model, route), factory)
//                    .SelectMany(tuple => tuple.Item1.RootElements.Single().As<RailwayContainer>().Descendants().OfType<IRoute>(),
//                        (tuple, route) => new ModelElementTuple<IRoute, IRoute>(tuple.Item2, route))
//                    .Where(tuple => tuple.Item1 != tuple.Item2 && tuple.Item2.Entry != tuple.Item1.Exit)
